...
g_even=[] #contenedor de la transformada inversa parte par;
g_odd=[] #contenedor de la transformada inversa parte impar;

initial_time=time.time() #cronometro que arranca en tiempo t=0s;
for j in range(N):
    Int=0 #contador de la integral en 0;
    for k in range(int(N/2)-1):
        Int+=f[2*k]*np.exp(-1j*(2*np.pi*2*k*j/N)) #IFFT parte par;
    g_even.append(Int) #agregar en el contenedor g_even parte par;

for j in range(N):
    Int=0 #contador de la integral en 0;
    for k in range(int(N/2)-1):
        Int+=f[2*k+1]*np.exp(-1j*(2*np.pi*(2*k+1)*j/N)) #IFFT parte impar;
    g_odd.append(Int) #agregar en el contenedor g_odd parte impar;
    
g_even=np.array(g_even) #creación de array g_even con lista g_even;
g_odd=np.array(g_odd) #creación de array g_odd con lista g_odd;
g_total=(1/np.sqrt(N))*(g_even+g_odd) #IFFT total;
...
print("El tiempo total gastado en hacer FFT fue de:",end=" ")
